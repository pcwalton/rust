...
and referent_type
  ...
  let sv b = s (v b) in
  let sp t = s (p t) in
  let recur ty = referent_type ?parent_tag:parent_tag ~boxed:boxed cx ty in
  ...
  let tag ttag =
    let n = get_n_tag_tups cx ttag in
    let union =
      let rty t = referent_type ~parent_tag:ttag ~boxed:boxed cx t in
  ...
  in
    ...
    match t with
      ...
      | Ast.TY_tag ttag ->
          begin
            match parent_tag with
                Some parent_tag when boxed && is_back_edge ttag parent_tag ->
      ...
      | Ast.TY_box t ->
          sp (Il.StructTy
            [| word; referent_type ?parent_tag:parent_tag ~boxed:true cx t |])
