...
pub fn noop_fold_expr<T:ast_fold>(e: @ast::Expr, folder: &T) -> @ast::Expr {
    ...
    let node = match e.node {
        ...
        ExprCast(expr, ref ty) => {
            ExprCast(folder.fold_expr(expr), (*ty).clone())
        }
        ...
        ExprBreak(ref opt_ident) => {
            ExprBreak(opt_ident.map_move(|x| folder.fold_ident(x)))
        }
        ExprAgain(ref opt_ident) => {
            ExprAgain(opt_ident.map_move(|x| folder.fold_ident(x)))
        }
        ExprRet(ref e) => {
            ExprRet(e.map_move(|x| folder.fold_expr(x)))
        }
        ExprLog(lv, e) => {
            ExprLog(folder.fold_expr(lv), folder.fold_expr(e))
        }
        ExprInlineAsm(ref a) => {
