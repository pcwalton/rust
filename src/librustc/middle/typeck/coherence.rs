...
pub fn CoherenceChecker<'a>(crate_context: &'a CrateCtxt)
                        -> CoherenceChecker<'a> {
    CoherenceChecker {
...
struct PrivilegedScopeVisitor<'c> {
    cc: CoherenceChecker<'c>,
}

impl<'c> visit::Visitor<()> for PrivilegedScopeVisitor<'c> {
    fn visit_item(&mut self, item: &item, _: ()) {
