...
impl TypedArenaChunk {
    ...
    fn start(&self, tydesc: *TyDesc) -> *u8 {
        unsafe {
            ...
            let start: uint = cast::transmute(this.offset(1));
            cast::transmute(round_up(start, (*tydesc).align))
        }
    }
...
impl<T> TypedArena<T> {
    ...
    unsafe fn grow(&self) {
        let this = cast::transmute_mut(self);
        let chunk = util::replace(&mut this.first, intrinsics::uninit());
...
impl<T> Drop for TypedArena<T> {
    fn drop(&mut self) {
        unsafe {
            ...
            self.first.destroy(diff, opt_tydesc)
        }
    }
}
