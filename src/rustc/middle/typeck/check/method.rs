...
class lookup {
    ...
    fn add_candidates_from_scope(use_assignability: bool) {
        ...
        if self.fcx.ccx.tcx.sess.coherence() &&
                self.fcx.ccx.trait_map.contains_key(self.expr.id) {
            ret;
        }
    ...
    fn add_inherent_and_extension_candidates(optional_inherent_methods:
        ...
        // Bail out if we aren't compiling with coherence.
        if !self.fcx.ccx.tcx.sess.coherence() {
            ret;
        }
            ...
            some(trait_ids) {
                for (*trait_ids).each |trait_id| {
                    ...
                    alt coherence_info.extension_methods.find(trait_id) {
                        none {
                            // XXX: Should be span_bug.
                            self.fcx.ccx.tcx.sess.span_warn(self.expr.span,
                                                            "no extension \
                                                             methods found \
                                                             for this trait");
                        }
